...
// PortConfig.center ± mockAreaRadiusKm 사각형을 bounding box(대각 꼭짓점 2개)로 변환한다.
...
export function boundingBoxAround(config: PortConfig): BoundingBox {
  const { center, mockAreaRadiusKm } = config;
  const latDeg = mockAreaRadiusKm / 111; // 1도 ≈ 111km
  const lonDeg = mockAreaRadiusKm / (111 * Math.cos((center.lat * Math.PI) / 180));
  return [
