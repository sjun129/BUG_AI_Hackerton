...
import type { ClimateOverrideInput, RouteScenarioResponse } from "@/frontend/types/route-scenario";
...
export function useRouteScenarios() {
  ...
  const calculateRouteScenarios = useCallback(
    async (scenarioShips: SimulatedShip[], climateOverride?: ClimateOverrideInput) => {
      setError(null);
      setNotice(null);
      ...
      try {
        const response = await fetch("/api/route-scenarios", {
          ...
          body: JSON.stringify({
            ...
            scenarioShips,
            ...(climateOverride?.enabled ? { climateOverride } : {}),
          }),
        });
